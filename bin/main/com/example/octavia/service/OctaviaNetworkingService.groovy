...
class OctaviaNetworkingService {
     ...
     */
    ServiceResponse createFloatingIp(Cloud cloud, String projectId, String floatingNetworkId) {
        try {
            OctaviaApiClient client = getClient(cloud, projectId)
            Map payload = [
                floatingip: [
            ...
            ServiceResponse response = client.post("/v2.0/floatingips", payload)
            if (response.success) {
                return ServiceResponse.success(response.data?.floatingip)
            } else {
                return ServiceResponse.error("Create Floating IP failed: ${response.msg ?: response.error}")
            }
        } catch (Exception e) {
            log.error("Error creating floating IP: ${e.message}", e)
            return ServiceResponse.error("Error creating floating IP: ${e.message}")
        }
    }
